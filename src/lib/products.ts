...
const builtInSeeds: BuiltInSeed[] = [
  {
    ...
    highlights: [
      "Small, mini, and large sizes",
      "Easy-to-read barrel markings",
      "Individually prepared for reliable handling",
    ],
    options: [
  ...
  },
  {
    ...
    highlights: [
      ...
      "Built for GXZ reusable pens",
      "Compact set for easy stocking",
    ],
    options: [],
    inStock: true,
    isActive: true,
  ...
  },
  {
    ...
    highlights: [
      ...
      "Compact, easy-to-store packaging",
    ],
  ...
  {
    slug: "body-balm",
    name: "GXZ Peptides Nourishing Body Balm",
    description:
  ...
  {
    slug: "creatine",
    name: "GXZ Peptides Creatine Performance Matrix Powder",
    description:
    ...
    highlights: [
      "Easy daily performance support",
      "Mixes smoothly",
      "Clean supplement profile",
    ],
    options: [],
    inStock: true,
    isActive: true,
...
function optionArrayFromJson(value: Json | null | undefined) {
  ...
  return value.flatMap((entry) => {
    ...
    const rawType =
      ...
      ["color", "size", "general"].includes(entry.type)
        ? (entry.type as ProductOption["type"])
        : undefined;
    const rawInStock =
      typeof entry.inStock === "boolean" ? entry.inStock : undefined;
...
function normalizePenOptions(options: ProductOption[]) {
  const optionsByValue = new Map(
    penColorOptions.map((option) => [option.value, option]),
  );
  ...
  options.forEach((option) => {
    if (
      option.type === "size" ||
      ["4mm", "8mm", "32g-4mm", "31g-8mm"].includes(option.value)
    ) {
      return;
    }
  ...
  return [
    ...penColorOptions.map(
      (option) => optionsByValue.get(option.value) ?? option,
    ),
    ...customOptions,
  ].filter(
    (option, index, list) =>
...
function normalizeNeedleOptions(options: ProductOption[]) {
  const optionsByValue = new Map(
    needleSizeOptions.map((option) => [option.value, option]),
  );
  ...
  return needleSizeOptions.map(
    (option) => optionsByValue.get(option.value) ?? option,
  );
}

function galleryFromRecord(
  record: Pick<ProductRecord, "slug" | "image_url" | "gallery">,
) {
  const builtIn = builtInMedia[record.slug];
  const customGallery = stringArrayFromJson(record.gallery);
  const primaryImage =
    record.image_url?.trim() || builtIn?.image || syringeImage;

  if (customGallery.length > 0) {
    return [
      primaryImage,
      ...customGallery.filter((entry) => entry !== primaryImage),
    ];
  }
...
export function normalizeCatalogProduct(record: ProductRecord): CatalogProduct {
  ...
  return {
    ...
    longDescription:
      record.long_description || builtIn?.longDescription || record.description,
    price: Number(record.price),
    category: record.category,
    image: record.image_url?.trim() || builtInVisuals?.image || syringeImage,
    gallery: galleryFromRecord(record),
    features:
      stringArrayFromJson(record.features).length > 0
        ? stringArrayFromJson(record.features)
        : (builtIn?.features ?? []),
    highlights:
      stringArrayFromJson(record.highlights).length > 0
